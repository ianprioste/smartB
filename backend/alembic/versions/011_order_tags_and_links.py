"""Add order tags and links tables
...
def upgrade() -> None:
    ...
    if not _table_exists(conn, "order_tags"):
        op.create_table(
            ...
            sa.UniqueConstraint("tenant_id", "scope_key", "event_id", "name_key", name="uq_order_tags_tenant_scope_event_name"),
        )
    ...
    if not _table_exists(conn, "order_tag_assignments"):
        op.create_table(
            ...
            sa.Column("created_at", sa.DateTime(), nullable=False, server_default=sa.func.now()),
            sa.Column("updated_at", sa.DateTime(), nullable=False, server_default=sa.func.now()),
            sa.UniqueConstraint("tenant_id", "scope_key", "event_id", "bling_order_id", name="uq_order_tag_assignments_tenant_scope_event_order"),
        )

    if not _index_exists(conn, "order_tag_assignments", "ix_order_tag_assignments_scope"):
        op.create_index("ix_order_tag_assignments_scope", "order_tag_assignments", ["tenant_id", "scope_key", "event_id", "bling_order_id"], unique=False)
    ...
    if not _index_exists(conn, "order_tag_links", "ix_order_tag_links_scope"):
        op.create_index("ix_order_tag_links_scope", "order_tag_links", ["tenant_id", "scope_key", "event_id", "bling_order_id"], unique=False)
...
def downgrade() -> None:
    ...
    if _table_exists(conn, "order_tag_links"):
        if _index_exists(conn, "order_tag_links", "ix_order_tag_links_scope"):
            op.drop_index("ix_order_tag_links_scope", table_name="order_tag_links")
        op.drop_table("order_tag_links")

    if _table_exists(conn, "order_tag_assignments"):
        if _index_exists(conn, "order_tag_assignments", "ix_order_tag_assignments_scope"):
            op.drop_index("ix_order_tag_assignments_scope", table_name="order_tag_assignments")
